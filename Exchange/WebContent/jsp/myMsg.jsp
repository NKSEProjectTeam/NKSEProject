...
function agree(id) {
	sendRequest("/Exchange/ConfirmServlet?type=agree&id=" + id);
	document.getElementById(id).innerHTML = "已同意";
}

function refuse(id) {
	sendRequest("/Exchange/ConfirmServlet?type=refuse&id=" + id);
	document.getElementById(id).innerHTML = "已拒绝";
}
			...
			<div class="grid_product">
				...
				<h1>这里添加主内容！！！</h1>
					<table class="cart_product">
						<tr>
						    <th class="bg name" style="width: 100px;">我的商品名称</th>
						    <th class="edit" style="width: 100px;">交换方</th>
						    <th class="bg price" style="width: 100px;">交换商品名称</th>
						    <th class="qty" style="width: 100px;">发布时间</th>
						    <th class="qty" style="width: 100px;"></th>
						</tr>
						<c:if test="${!empty sessionScope.lstWO }">
						<c:forEach items="${sessionScope.lstWO }" var="wo">
							<tr>
								<form method="post" action="/Exchange/TradeServlet">
								<input type="hidden" name="id" value="${wo.bid }">
								<td width="100" class="bg name">${wo.name1 }</td>
								<td width="100" class="edit">${wo.username }</td>
								<td width="100" class="bg price">${wo.name2 }</td>
								<td width="100" class="qty" style="align:center;">${wo.posttime }</td>
								<td width="100" id="${wo.bid }" class="qty" style="align:center;"><input type="button" onclick="agree(${wo.bid })" value="同意">&nbsp<input type="button" onclick="refuse(${wo.bid })" value="拒绝"></td>
								</form>
							</tr>
						</c:forEach>
						</c:if>
