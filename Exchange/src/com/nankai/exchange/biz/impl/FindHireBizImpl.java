...
public class FindHireBizImpl implements IFindHireBiz {

	public List<List<?>> findHireState(String msg) {
     	...
     	for(int i = 0; i < types.length; i++) {
			...
			case "dailypros":
				...
				if(lstex6.size() < 8) {
					...
					lst = lst.subList(lstex6.size() - 8, lstex6.size());
					lstStateZero.add(lstex6);
				}
				break;
			case "entertainments":
				...
				for(int j=0;j<lst.size();j++)
				{
					...
					if(entertainments.getEnterstate()==1){
				...
				if(lstex7.size() < 8) {
					...
					lst = lst.subList(lstex7.size() - 8, lstex7.size());
					lstStateZero.add(lstex7);
				}
				break;
			case "electrics":
